...
const GRAVITY_FALL  = 1.15;   // plus lourde en descente → chute nette
const GRAVITY_RISE  = 0.44;   // plus légère en montée (bouton maintenu)
const GRAVITY_CUT   = 0.65;   // si on lâche saut tôt → coupe la montée
const MAX_FALL      = 20;
const PLAYER_SPMAX  = 5.8;
const PLAYER_ACCEL  = 1.9;    // accél sol
...
let quizTimer      = 0;
let currentQuiz    = null;   // randomly picked from QUIZ_POOL each level-end
...
let levelData      = null;
let player         = null;
let checkpointX    = 0;   // respawn X after checkpoint
let checkpointY    = 0;   // respawn Y after checkpoint
...
let _endMouseX     = -1;  // mouse position for hover rendering only
let _endMouseY     = -1;
...
// Endscreen button positions (canvas coords, VW=960 VH=540)
const END_CTA = { x:260, y:295, w:440, h:62 };
const END_RPL = { x:370, y:374, w:220, h:50 };

// Called DIRECTLY from user-gesture handlers (click / touchstart)
// so window.open() is allowed by browser popup policy.
function _endScreenAction(cx, cy) {
  if (cx >= END_CTA.x && cx < END_CTA.x+END_CTA.w &&
      cy >= END_CTA.y && cy < END_CTA.y+END_CTA.h) {
...
// Desktop click — direct user gesture
canvas.addEventListener('click', e => {
  if (gameState !== 'endscreen') return;
  const r = canvas.getBoundingClientRect();
  const cx = (e.clientX - r.left) * VW / r.width;
  const cy = (e.clientY - r.top)  * VH / r.height;
  _endScreenAction(cx, cy);
});

// Desktop hover — for visual feedback only
canvas.addEventListener('mousemove', e => {
  if (gameState !== 'endscreen') return;
  const r = canvas.getBoundingClientRect();
  _endMouseX = (e.clientX - r.left) * VW / r.width;
  _endMouseY = (e.clientY - r.top)  * VH / r.height;
});
...
const touch = (() => {
  ...
  canvas.addEventListener('touchstart', e => {
    ...
    for (const t of e.changedTouches) {
      ...
      if (gameState === 'endscreen') { _endScreenAction(cx, cy); continue; }
...
class Player {
  ...
  _resolveX() {
    for (const p of [...lPlatforms, ...lMoving]) {
...
class MovingPlatform {
  constructor(x, y, w, h, type, axis, dist, speed) {
    this.x = x; this.y = y; this.w = w; this.h = h; this.type = type;
    this.ox = x; this.oy = y;    // origin
    this.axis = axis;            // 'x' or 'y'
    this.dist = dist;            // travel distance (px)
    this.speed = speed;          // px per frame
    this.dir = 1;
    this.dx = 0; this.dy = 0;   // delta this frame (for player riding)
  }
  update() {
    const prev = this.axis === 'x' ? this.x : this.y;
    if (this.axis === 'x') {
      this.x += this.speed * this.dir;
      if (this.x > this.ox + this.dist || this.x < this.ox) this.dir *= -1;
      this.dx = this.x - prev; this.dy = 0;
    } else {
      this.y += this.speed * this.dir;
      if (this.y > this.oy + this.dist || this.y < this.oy) this.dir *= -1;
      this.dx = 0; this.dy = this.y - prev;
    }
  }
...
class Checkpoint {
  ...
  update() { this.pulse = (this.pulse + 0.08) % (Math.PI * 2); }
  draw() {
    ctx.save(); ctx.translate(this.x + 14, this.y);
    // Pole
    ctx.fillStyle = this.activated ? '#FFD700' : '#888';
    ctx.fillRect(-3, 0, 6, 48);
    // Flag
    const fc = this.activated ? '#FFD700' : '#AAAAAA';
    const wave = this.activated ? Math.sin(this.pulse) * 6 : 0;
    ctx.fillStyle = fc;
    ctx.beginPath();
    ctx.moveTo(3, 4); ctx.lineTo(3 + 22, 10 + wave);
    ctx.lineTo(3 + 22, 22 + wave); ctx.lineTo(3, 22);
    ctx.closePath(); ctx.fill();
    // Glow when activated
    if (this.activated) {
      const glow = ctx.createRadialGradient(0, 24, 0, 0, 24, 30 + Math.sin(this.pulse) * 5);
      glow.addColorStop(0, 'rgba(255,215,0,0.25)');
      glow.addColorStop(1, 'rgba(255,215,0,0)');
      ctx.fillStyle = glow; ctx.beginPath(); ctx.arc(0, 24, 40, 0, Math.PI * 2); ctx.fill();
    }
    ctx.restore();
  }
}
...
// ============================================================
const LEVELS = [
  // ── NIVEAU 0 : La Chocolaterie (tutoriel, facile) ────────────────
  {
    ...
    platforms: [
      // Sol principal
      { x:0,    y:460, w:600,  h:20, type:'ground'    },
      { x:600,  y:460, w:400,  h:20, type:'chocolate' },
      { x:1000, y:460, w:400,  h:20, type:'cream'     },
      { x:1400, y:460, w:400,  h:20, type:'nougat'    },
      { x:1800, y:460, w:500,  h:20, type:'ground'    },
      { x:2300, y:460, w:500,  h:20, type:'chocolate' },
      // Plateformes montantes progressives
      { x:200,  y:380, w:120,  h:16, type:'chocolate' },
      { x:400,  y:320, w:120,  h:16, type:'cream'     },
      { x:560,  y:380, w:100,  h:16, type:'nougat'    },
      { x:720,  y:300, w:140,  h:16, type:'chocolate' },
      { x:900,  y:380, w:120,  h:16, type:'cream'     },
      { x:1080, y:300, w:120,  h:16, type:'candy'     },
      { x:1260, y:380, w:100,  h:16, type:'ground'    },
      { x:1440, y:300, w:140,  h:16, type:'nougat'    },
      { x:1640, y:360, w:120,  h:16, type:'chocolate' },
      { x:1820, y:290, w:140,  h:16, type:'cream'     },
      { x:2020, y:360, w:100,  h:16, type:'candy'     },
      { x:2180, y:290, w:140,  h:16, type:'nougat'    },
      { x:2380, y:360, w:120,  h:16, type:'ground'    },
      { x:2550, y:290, w:140,  h:16, type:'chocolate' },
      { x:2720, y:350, w:80,   h:16, type:'cream'     },
    ],
    moving: [
      { x:660, y:380, w:80, h:16, type:'candy', axis:'x', dist:80, speed:1.2 },
      { x:1560, y:360, w:80, h:16, type:'nougat', axis:'y', dist:60, speed:0.8 },
    ],
    checkpoints: [
      { x:900, y:340 },
      { x:1800, y:340 },
    ...
    collectibles: [
      // Ligne de collectibles qui guide le joueur
      ...[80,160,240,320,400,480,560,640,720,800,880,960,1040,1120,1200,1280,1360,1440,
          1520,1600,1680,1760,1840,1920,2000,2080,2160,2240,2320,2400,2480,2560,2640,2720].map(
        (x,i) => ({ x, y: 260 + (i%3)*40 })
      ),
    ],
    powerups: [
      { x:440,  y:280, w:32, h:32, type:'speed'      },
      { x:1100, y:260, w:32, h:32, type:'doubleJump' },
      { x:1860, y:250, w:32, h:32, type:'shield'     },
      { x:2560, y:250, w:32, h:32, type:'speed'      },
    ],
    enemies: [
      { x:700,  y:440, type:'chocolate', range:140 },
      { x:1100, y:440, type:'chocolate', range:100 },
      { x:1500, y:440, type:'hazelnut'             },
      { x:2100, y:440, type:'chocolate', range:120 },
    ],
    endZone: { x:2720, y:370, w:56, h:80 },
  },

  // ── NIVEAU 1 : Grotte de Noisettes (sauts & wall-jump) ──────────
  {
    name: 'Grotte de Noisettes',
    width: 3800, height: 650,
    bgTop: '#4A2800', bgBot: '#2A1000',
    spawn: { x: 80, y: 450 },
    platforms: [
      { x:0,    y:520, w:450,  h:20, type:'nougat'    },
      { x:500,  y:520, w:300,  h:20, type:'ground'    },
      { x:850,  y:520, w:300,  h:20, type:'chocolate' },
      { x:1200, y:520, w:350,  h:20, type:'nougat'    },
      { x:1600, y:520, w:300,  h:20, type:'cream'     },
      { x:1950, y:520, w:350,  h:20, type:'nougat'    },
      { x:2350, y:520, w:300,  h:20, type:'ground'    },
      { x:2700, y:520, w:350,  h:20, type:'chocolate' },
      { x:3100, y:520, w:300,  h:20, type:'nougat'    },
      { x:3450, y:520, w:350,  h:20, type:'cream'     },
      // Plateformes murales (pour wall-jump)
      { x:460,  y:420, w:40,   h:100,type:'nougat'    },
      { x:460,  y:300, w:40,   h:80, type:'nougat'    },
      { x:800,  y:380, w:40,   h:90, type:'nougat'    },
      { x:800,  y:260, w:40,   h:70, type:'nougat'    },
      // Escalier ascendant
      { x:220,  y:450, w:100,  h:16, type:'chocolate' },
      { x:360,  y:380, w:100,  h:16, type:'cream'     },
      { x:520,  y:310, w:100,  h:16, type:'nougat'    },
      { x:660,  y:240, w:120,  h:16, type:'chocolate' },
      { x:840,  y:310, w:100,  h:16, type:'cream'     },
      { x:980,  y:380, w:120,  h:16, type:'nougat'    },
      { x:1120, y:300, w:100,  h:16, type:'candy'     },
      { x:1280, y:380, w:120,  h:16, type:'chocolate' },
      { x:1440, y:300, w:100,  h:16, type:'cream'     },
      { x:1600, y:400, w:100,  h:16, type:'nougat'    },
      { x:1760, y:320, w:120,  h:16, type:'chocolate' },
      { x:1940, y:400, w:100,  h:16, type:'candy'     },
      { x:2100, y:320, w:120,  h:16, type:'cream'     },
      { x:2280, y:400, w:100,  h:16, type:'nougat'    },
      { x:2460, y:320, w:120,  h:16, type:'ground'    },
      { x:2620, y:400, w:100,  h:16, type:'chocolate' },
      { x:2780, y:310, w:120,  h:16, type:'cream'     },
      { x:2940, y:390, w:100,  h:16, type:'nougat'    },
      { x:3100, y:310, w:120,  h:16, type:'candy'     },
      { x:3260, y:390, w:100,  h:16, type:'ground'    },
      { x:3400, y:310, w:120,  h:16, type:'chocolate' },
    ],
    moving: [
      { x:1340, y:440, w:90, h:16, type:'candy', axis:'x', dist:100, speed:1.5 },
      { x:2040, y:360, w:90, h:16, type:'nougat', axis:'y', dist:80,  speed:1.0 },
      { x:2660, y:380, w:90, h:16, type:'cream', axis:'x',  dist:120, speed:1.8 },
      { x:3200, y:350, w:90, h:16, type:'candy',  axis:'y', dist:60,  speed:1.2 },
    ],
    checkpoints: [
      { x:1100, y:480 },
      { x:2300, y:480 },
    ...
    collectibles: [
      ...[100,180,260,340,420,500,600,700,800,900,1000,1100,1200,1300,1400,
          1500,1600,1700,1800,1900,2000,2100,2200,2300,2400,2500,2600,2700,
          2800,2900,3000,3100,3200,3300,3400,3500,3600,3700].map(
        (x,i) => ({ x, y: 220 + (i%4)*50 })
      ),
    ],
    powerups: [
      { x:560,  y:270, w:32, h:32, type:'doubleJump' },
      { x:1140, y:260, w:32, h:32, type:'speed'      },
      { x:1800, y:280, w:32, h:32, type:'dash'       },
      { x:2480, y:280, w:32, h:32, type:'shield'     },
      { x:3140, y:270, w:32, h:32, type:'ultra'      },
    ],
    enemies: [
      { x:600,  y:500, type:'chocolate', range:140 },
      { x:900,  y:500, type:'hazelnut'             },
      { x:1250, y:500, type:'chocolate', range:120 },
      { x:1650, y:500, type:'guard',     range:200 },
      { x:2000, y:500, type:'hazelnut'             },
      { x:2400, y:500, type:'chocolate', range:160 },
      { x:2750, y:500, type:'guard',     range:220 },
      { x:3150, y:500, type:'hazelnut'             },
      { x:3500, y:500, type:'guard',     range:200 },
    ],
    endZone: { x:3700, y:420, w:56, h:80 },
  },

  // ── NIVEAU 2 : Les Pics Sucrés (plateformes mobiles intenses) ───
  {
    name: 'Les Pics Sucrés',
    width: 4400, height: 650,
    bgTop: '#001A3A', bgBot: '#003366',
    spawn: { x: 80, y: 420 },
    platforms: [
      { x:0,    y:500, w:350,  h:20, type:'candy'     },
      { x:400,  y:500, w:250,  h:20, type:'chocolate' },
      { x:700,  y:500, w:250,  h:20, type:'cream'     },
      { x:1000, y:500, w:300,  h:20, type:'nougat'    },
      { x:1350, y:500, w:250,  h:20, type:'candy'     },
      { x:1650, y:500, w:300,  h:20, type:'chocolate' },
      { x:2000, y:500, w:250,  h:20, type:'cream'     },
      { x:2300, y:500, w:300,  h:20, type:'nougat'    },
      { x:2650, y:500, w:250,  h:20, type:'candy'     },
      { x:2950, y:500, w:300,  h:20, type:'chocolate' },
      { x:3300, y:500, w:250,  h:20, type:'cream'     },
      { x:3600, y:500, w:300,  h:20, type:'nougat'    },
      { x:3950, y:500, w:450,  h:20, type:'candy'     },
      // Hautes plateformes bonus
      { x:300,  y:380, w:100,  h:16, type:'candy'     },
      { x:500,  y:310, w:100,  h:16, type:'cream'     },
      { x:750,  y:380, w:100,  h:16, type:'nougat'    },
      { x:1000, y:300, w:100,  h:16, type:'chocolate' },
      { x:1200, y:380, w:100,  h:16, type:'candy'     },
      { x:1500, y:300, w:100,  h:16, type:'cream'     },
      { x:1750, y:380, w:100,  h:16, type:'nougat'    },
      { x:2050, y:300, w:100,  h:16, type:'chocolate' },
      { x:2350, y:380, w:100,  h:16, type:'candy'     },
      { x:2700, y:300, w:100,  h:16, type:'cream'     },
      { x:3000, y:380, w:100,  h:16, type:'nougat'    },
      { x:3350, y:300, w:100,  h:16, type:'chocolate' },
      { x:3650, y:380, w:100,  h:16, type:'candy'     },
      { x:4050, y:300, w:100,  h:16, type:'cream'     },
      { x:4250, y:380, w:100,  h:16, type:'nougat'    },
    ],
    moving: [
      // Séquence de 8 plateformes mobiles progressivement plus rapides
      { x:355,  y:470, w:80, h:16, type:'candy',    axis:'x', dist:40,  speed:1.0 },
      { x:655,  y:470, w:80, h:16, type:'cream',    axis:'y', dist:70,  speed:1.2 },
      { x:1005, y:460, w:80, h:16, type:'nougat',   axis:'x', dist:100, speed:1.5 },
      { x:1355, y:460, w:80, h:16, type:'candy',    axis:'y', dist:80,  speed:1.4 },
      { x:1960, y:460, w:80, h:16, type:'chocolate',axis:'x', dist:120, speed:1.8 },
      { x:2255, y:460, w:80, h:16, type:'cream',    axis:'y', dist:90,  speed:1.6 },
      { x:2655, y:460, w:80, h:16, type:'nougat',   axis:'x', dist:140, speed:2.0 },
      { x:2955, y:450, w:80, h:16, type:'candy',    axis:'y', dist:100, speed:2.0 },
      { x:3305, y:450, w:80, h:16, type:'chocolate',axis:'x', dist:150, speed:2.2 },
      { x:3605, y:450, w:80, h:16, type:'cream',    axis:'y', dist:110, speed:2.2 },
    ],
    checkpoints: [
      { x:900,  y:460 },
      { x:1900, y:460 },
    ...
    collectibles: [
      ...[120,200,280,360,440,520,620,720,820,920,1020,1120,1220,1320,1420,
          1520,1620,1720,1820,1920,2020,2120,2220,2320,2420,2520,2620,2720,
          2820,2920,3020,3120,3220,3320,3420,3520,3620,3720,3820,3920,4020,
          4120,4220,4320].map(
        (x,i) => ({ x, y: 240 + (i%5)*38 })
      ),
    ],
    powerups: [
      { x:500,  y:270, w:32, h:32, type:'doubleJump' },
      { x:1000, y:260, w:32, h:32, type:'speed'      },
      { x:1750, y:260, w:32, h:32, type:'dash'       },
      { x:2700, y:260, w:32, h:32, type:'shield'     },
      { x:3350, y:260, w:32, h:32, type:'ultra'      },
      { x:4060, y:260, w:32, h:32, type:'doubleJump' },
    ],
    enemies: [
      { x:420,  y:480, type:'chocolate', range:160 },
      { x:720,  y:480, type:'hazelnut'             },
      { x:1020, y:480, type:'guard',     range:220 },
      { x:1370, y:480, type:'hazelnut'             },
      { x:1670, y:480, type:'chocolate', range:180 },
      { x:2020, y:480, type:'guard',     range:240 },
      { x:2320, y:480, type:'hazelnut'             },
      { x:2670, y:480, type:'chocolate', range:200 },
      { x:2970, y:480, type:'guard',     range:260 },
      { x:3320, y:480, type:'hazelnut'             },
      { x:3620, y:480, type:'chocolate', range:220 },
      { x:3970, y:480, type:'guard',     range:280 },
    ],
    endZone: { x:4330, y:380, w:56, h:80 },
  },

  // ── NIVEAU 3 : Caverne de Lave ──────────────────────────────────
  {
    name: 'Caverne de Lave',
    width: 4600, height: 680,
    bgTop: '#1A0000', bgBot: '#5C1200',
    spawn: { x: 80, y: 420 },
    platforms: [
      { x:0,    y:540, w:280, h:20, type:'nougat'    },
      { x:330,  y:540, w:220, h:20, type:'chocolate' },
      { x:600,  y:540, w:220, h:20, type:'nougat'    },
      { x:870,  y:540, w:220, h:20, type:'chocolate' },
      { x:1140, y:540, w:220, h:20, type:'nougat'    },
      { x:1410, y:540, w:220, h:20, type:'chocolate' },
      { x:1680, y:540, w:220, h:20, type:'nougat'    },
      { x:1950, y:540, w:220, h:20, type:'chocolate' },
      { x:2220, y:540, w:220, h:20, type:'nougat'    },
      { x:2490, y:540, w:220, h:20, type:'chocolate' },
      { x:2760, y:540, w:220, h:20, type:'nougat'    },
      { x:3030, y:540, w:220, h:20, type:'chocolate' },
      { x:3300, y:540, w:220, h:20, type:'nougat'    },
      { x:3570, y:540, w:220, h:20, type:'chocolate' },
      { x:3840, y:540, w:220, h:20, type:'nougat'    },
      { x:4110, y:540, w:490, h:20, type:'chocolate' },
      // Plateformes intermédiaires
      { x:140,  y:450, w:100, h:16, type:'candy'     },
      { x:280,  y:370, w:110, h:16, type:'chocolate' },
      { x:430,  y:450, w:100, h:16, type:'nougat'    },
      { x:580,  y:370, w:100, h:16, type:'candy'     },
      { x:740,  y:450, w:110, h:16, type:'chocolate' },
      { x:910,  y:370, w:100, h:16, type:'nougat'    },
      { x:1080, y:450, w:110, h:16, type:'candy'     },
      { x:1240, y:370, w:100, h:16, type:'chocolate' },
      { x:1400, y:450, w:100, h:16, type:'nougat'    },
      { x:1560, y:370, w:110, h:16, type:'candy'     },
      { x:1740, y:300, w:120, h:16, type:'chocolate' },
      { x:1920, y:380, w:100, h:16, type:'nougat'    },
      { x:2100, y:300, w:120, h:16, type:'candy'     },
      { x:2280, y:380, w:100, h:16, type:'chocolate' },
      { x:2460, y:300, w:120, h:16, type:'nougat'    },
      { x:2640, y:380, w:110, h:16, type:'candy'     },
      { x:2820, y:300, w:120, h:16, type:'chocolate' },
      { x:3010, y:380, w:100, h:16, type:'nougat'    },
      { x:3200, y:300, w:120, h:16, type:'candy'     },
      { x:3390, y:380, w:100, h:16, type:'chocolate' },
      { x:3580, y:300, w:120, h:16, type:'nougat'    },
      { x:3760, y:380, w:100, h:16, type:'candy'     },
      { x:3940, y:300, w:120, h:16, type:'chocolate' },
      { x:4120, y:380, w:100, h:16, type:'nougat'    },
      { x:4280, y:300, w:120, h:16, type:'candy'     },
    ],
    moving: [
      { x:290,  y:500, w:80, h:16, type:'candy',    axis:'x', dist:30,  speed:2.0 },
      { x:620,  y:500, w:80, h:16, type:'nougat',   axis:'x', dist:50,  speed:2.2 },
      { x:950,  y:490, w:80, h:16, type:'chocolate',axis:'y', dist:80,  speed:1.8 },
      { x:1280, y:490, w:80, h:16, type:'candy',    axis:'x', dist:60,  speed:2.4 },
      { x:1600, y:490, w:80, h:16, type:'nougat',   axis:'y', dist:100, speed:2.0 },
      { x:1940, y:480, w:80, h:16, type:'chocolate',axis:'x', dist:70,  speed:2.6 },
      { x:2280, y:480, w:80, h:16, type:'candy',    axis:'y', dist:110, speed:2.2 },
      { x:2620, y:470, w:80, h:16, type:'nougat',   axis:'x', dist:80,  speed:2.8 },
      { x:2970, y:470, w:80, h:16, type:'chocolate',axis:'y', dist:120, speed:2.4 },
      { x:3310, y:460, w:80, h:16, type:'candy',    axis:'x', dist:90,  speed:3.0 },
      { x:3660, y:460, w:80, h:16, type:'nougat',   axis:'y', dist:130, speed:2.6 },
      { x:4000, y:450, w:80, h:16, type:'chocolate',axis:'x', dist:100, speed:3.0 },
    ],
    checkpoints: [
      { x:850,  y:500 },
      { x:1900, y:500 },
    ...
    collectibles: [
      ...[100,180,260,340,420,500,580,660,740,820,900,980,1060,1140,1220,
          1300,1380,1460,1540,1620,1700,1780,1860,1940,2020,2100,2180,2260,
          2340,2420,2500,2580,2660,2740,2820,2900,2980,3060,3140,3220,3300,
          3380,3460,3540,3620,3700,3780,3860,3940,4020,4100,4180,4260,4340].map(
        (x,i) => ({ x, y: 230 + (i%5)*32 })
      ),
    ],
    powerups: [
      { x:440,  y:330, w:32, h:32, type:'speed'      },
      { x:900,  y:330, w:32, h:32, type:'doubleJump' },
      { x:1560, y:330, w:32, h:32, type:'dash'       },
      { x:2100, y:260, w:32, h:32, type:'shield'     },
      { x:2820, y:260, w:32, h:32, type:'ultra'      },
      { x:3580, y:260, w:32, h:32, type:'doubleJump' },
      { x:4280, y:260, w:32, h:32, type:'speed'      },
    ],
    enemies: [
      { x:350,  y:520, type:'chocolate', range:160 },
      { x:620,  y:520, type:'hazelnut'             },
      { x:890,  y:520, type:'guard',     range:200 },
      { x:1160, y:520, type:'chocolate', range:180 },
      { x:1430, y:520, type:'hazelnut'             },
      { x:1700, y:520, type:'guard',     range:220 },
      { x:1970, y:520, type:'chocolate', range:200 },
      { x:2240, y:520, type:'hazelnut'             },
      { x:2510, y:520, type:'guard',     range:240 },
      { x:2780, y:520, type:'chocolate', range:220 },
      { x:3050, y:520, type:'hazelnut'             },
      { x:3320, y:520, type:'guard',     range:260 },
      { x:3590, y:520, type:'chocolate', range:240 },
      { x:3860, y:520, type:'guard',     range:280 },
      { x:4130, y:520, type:'hazelnut'             },
    ],
    endZone: { x:4510, y:380, w:56, h:80 },
  },

  // ── NIVEAU 4 : Glacier du Sucre ─────────────────────────────────
  {
    name: 'Glacier du Sucre',
    width: 5000, height: 650,
    bgTop: '#B0D8FF', bgBot: '#2060A0',
    spawn: { x: 80, y: 400 },
    platforms: [
      // Grandes îles de glace espacées
      { x:0,    y:500, w:300, h:20, type:'candy'     },
      { x:380,  y:500, w:200, h:20, type:'cream'     },
      { x:660,  y:500, w:200, h:20, type:'candy'     },
      { x:940,  y:500, w:200, h:20, type:'cream'     },
      { x:1220, y:500, w:200, h:20, type:'candy'     },
      { x:1500, y:500, w:200, h:20, type:'cream'     },
      { x:1780, y:500, w:200, h:20, type:'candy'     },
      { x:2060, y:500, w:200, h:20, type:'cream'     },
      { x:2340, y:500, w:200, h:20, type:'candy'     },
      { x:2620, y:500, w:200, h:20, type:'cream'     },
      { x:2900, y:500, w:200, h:20, type:'candy'     },
      { x:3180, y:500, w:200, h:20, type:'cream'     },
      { x:3460, y:500, w:200, h:20, type:'candy'     },
      { x:3740, y:500, w:200, h:20, type:'cream'     },
      { x:4020, y:500, w:200, h:20, type:'candy'     },
      { x:4300, y:500, w:200, h:20, type:'cream'     },
      { x:4580, y:500, w:420, h:20, type:'candy'     },
      // Plateformes hautes (bonus + shortcuts)
      { x:250,  y:400, w:100, h:16, type:'cream'     },
      { x:450,  y:330, w:120, h:16, type:'candy'     },
      { x:700,  y:400, w:100, h:16, type:'cream'     },
      { x:900,  y:330, w:120, h:16, type:'candy'     },
      { x:1140, y:400, w:100, h:16, type:'cream'     },
      { x:1380, y:330, w:120, h:16, type:'candy'     },
      { x:1620, y:400, w:100, h:16, type:'cream'     },
      { x:1880, y:330, w:120, h:16, type:'candy'     },
      { x:2160, y:400, w:100, h:16, type:'cream'     },
      { x:2440, y:330, w:120, h:16, type:'candy'     },
      { x:2720, y:400, w:100, h:16, type:'cream'     },
      { x:3000, y:330, w:120, h:16, type:'candy'     },
      { x:3280, y:400, w:100, h:16, type:'cream'     },
      { x:3560, y:330, w:120, h:16, type:'candy'     },
      { x:3840, y:400, w:100, h:16, type:'cream'     },
      { x:4120, y:330, w:120, h:16, type:'candy'     },
      { x:4400, y:400, w:100, h:16, type:'cream'     },
    ],
    moving: [
      { x:300,  y:480, w:80, h:16, type:'candy',  axis:'x', dist:70,  speed:2.0 },
      { x:580,  y:480, w:80, h:16, type:'cream',  axis:'y', dist:90,  speed:2.0 },
      { x:860,  y:470, w:80, h:16, type:'candy',  axis:'x', dist:80,  speed:2.4 },
      { x:1140, y:470, w:80, h:16, type:'cream',  axis:'y', dist:110, speed:2.4 },
      { x:1420, y:460, w:80, h:16, type:'candy',  axis:'x', dist:90,  speed:2.8 },
      { x:1700, y:460, w:80, h:16, type:'cream',  axis:'y', dist:130, speed:2.8 },
      { x:1980, y:450, w:80, h:16, type:'candy',  axis:'x', dist:100, speed:3.0 },
      { x:2260, y:450, w:80, h:16, type:'cream',  axis:'y', dist:140, speed:3.0 },
      { x:2540, y:440, w:80, h:16, type:'candy',  axis:'x', dist:110, speed:3.2 },
      { x:2820, y:440, w:80, h:16, type:'cream',  axis:'y', dist:150, speed:3.2 },
      { x:3100, y:430, w:80, h:16, type:'candy',  axis:'x', dist:120, speed:3.4 },
      { x:3380, y:430, w:80, h:16, type:'cream',  axis:'y', dist:160, speed:3.4 },
      { x:3660, y:420, w:80, h:16, type:'candy',  axis:'x', dist:130, speed:3.6 },
      { x:3940, y:420, w:80, h:16, type:'cream',  axis:'y', dist:170, speed:3.6 },
      { x:4220, y:410, w:80, h:16, type:'candy',  axis:'x', dist:140, speed:3.8 },
    ],
    checkpoints: [
      { x:700,  y:460 },
      { x:1700, y:460 },
    ...
    collectibles: [
      ...[100,180,260,340,420,500,580,660,740,820,900,980,1060,1140,1220,
          1300,1380,1460,1540,1620,1700,1780,1860,1940,2020,2100,2180,2260,
          2340,2420,2500,2580,2660,2740,2820,2900,2980,3060,3140,3220,3300,
          3380,3460,3540,3620,3700,3780,3860,3940,4020,4100,4180,4260,4340,
          4420,4500,4580,4660,4740,4820,4900].map(
        (x,i) => ({ x, y: 240 + (i%6)*30 })
      ),
    ],
    powerups: [
      { x:460,  y:290, w:32, h:32, type:'doubleJump' },
      { x:900,  y:290, w:32, h:32, type:'speed'      },
      { x:1390, y:290, w:32, h:32, type:'dash'       },
      { x:1890, y:290, w:32, h:32, type:'shield'     },
      { x:2450, y:290, w:32, h:32, type:'ultra'      },
      { x:3010, y:290, w:32, h:32, type:'doubleJump' },
      { x:3570, y:290, w:32, h:32, type:'speed'      },
      { x:4130, y:290, w:32, h:32, type:'shield'     },
    ],
    enemies: [
      { x:400,  y:480, type:'hazelnut'             },
      { x:680,  y:480, type:'guard',     range:160 },
      { x:960,  y:480, type:'chocolate', range:180 },
      { x:1240, y:480, type:'hazelnut'             },
      { x:1520, y:480, type:'guard',     range:200 },
      { x:1800, y:480, type:'chocolate', range:200 },
      { x:2080, y:480, type:'hazelnut'             },
      { x:2360, y:480, type:'guard',     range:220 },
      { x:2640, y:480, type:'chocolate', range:220 },
      { x:2920, y:480, type:'hazelnut'             },
      { x:3200, y:480, type:'guard',     range:240 },
      { x:3480, y:480, type:'chocolate', range:240 },
      { x:3760, y:480, type:'hazelnut'             },
      { x:4040, y:480, type:'guard',     range:260 },
      { x:4320, y:480, type:'chocolate', range:260 },
      { x:4600, y:480, type:'guard',     range:280 },
    ],
    endZone: { x:4910, y:390, w:56, h:80 },
  },

  // ── NIVEAU 5 : Forêt Enchantée ───────────────────────────────────
  {
    name: 'Forêt Enchantée',
    width: 5400, height: 700,
    bgTop: '#020E00', bgBot: '#0A2800',
    spawn: { x: 80, y: 440 },
    platforms: [
      // Troncs et branches — montées / descentes
      { x:0,    y:540, w:300, h:20, type:'nougat'    },
      { x:350,  y:480, w:150, h:16, type:'ground'    },
      { x:550,  y:420, w:150, h:16, type:'nougat'    },
      { x:750,  y:360, w:150, h:16, type:'ground'    },
      { x:950,  y:300, w:150, h:16, type:'nougat'    },
      { x:1150, y:360, w:150, h:16, type:'ground'    },
      { x:1350, y:420, w:150, h:16, type:'nougat'    },
      { x:1550, y:480, w:150, h:16, type:'ground'    },
      { x:1750, y:540, w:200, h:20, type:'nougat'    },
      { x:2000, y:480, w:150, h:16, type:'ground'    },
      { x:2200, y:400, w:150, h:16, type:'nougat'    },
      { x:2400, y:320, w:150, h:16, type:'ground'    },
      { x:2600, y:260, w:150, h:16, type:'nougat'    },
      { x:2800, y:320, w:150, h:16, type:'ground'    },
      { x:3000, y:400, w:150, h:16, type:'nougat'    },
      { x:3200, y:480, w:150, h:16, type:'ground'    },
      { x:3400, y:540, w:200, h:20, type:'nougat'    },
      { x:3650, y:460, w:150, h:16, type:'ground'    },
      { x:3850, y:380, w:150, h:16, type:'nougat'    },
      { x:4050, y:300, w:150, h:16, type:'ground'    },
      { x:4250, y:380, w:150, h:16, type:'nougat'    },
      { x:4450, y:460, w:150, h:16, type:'ground'    },
      { x:4650, y:540, w:200, h:20, type:'nougat'    },
      { x:4900, y:460, w:150, h:16, type:'ground'    },
      { x:5100, y:380, w:150, h:16, type:'nougat'    },
      { x:5300, y:440, w:100, h:16, type:'ground'    },
      // Branches latérales bonus
      { x:450,  y:380, w:80,  h:16, type:'candy'     },
      { x:850,  y:260, w:80,  h:16, type:'candy'     },
      { x:1250, y:300, w:80,  h:16, type:'candy'     },
      { x:1650, y:380, w:80,  h:16, type:'candy'     },
      { x:2100, y:340, w:80,  h:16, type:'candy'     },
      { x:2500, y:220, w:80,  h:16, type:'candy'     },
      { x:2900, y:260, w:80,  h:16, type:'candy'     },
      { x:3300, y:380, w:80,  h:16, type:'candy'     },
      { x:3750, y:360, w:80,  h:16, type:'candy'     },
      { x:4150, y:260, w:80,  h:16, type:'candy'     },
      { x:4550, y:360, w:80,  h:16, type:'candy'     },
      { x:4950, y:380, w:80,  h:16, type:'candy'     },
    ],
    moving: [
      { x:310,  y:500, w:80, h:16, type:'nougat',   axis:'y', dist:60,  speed:2.0 },
      { x:720,  y:400, w:80, h:16, type:'ground',   axis:'x', dist:80,  speed:2.2 },
      { x:1100, y:340, w:80, h:16, type:'nougat',   axis:'y', dist:100, speed:2.4 },
      { x:1500, y:440, w:80, h:16, type:'ground',   axis:'x', dist:90,  speed:2.6 },
      { x:1870, y:500, w:80, h:16, type:'nougat',   axis:'y', dist:80,  speed:2.8 },
      { x:2280, y:360, w:80, h:16, type:'ground',   axis:'x', dist:100, speed:3.0 },
      { x:2680, y:280, w:80, h:16, type:'nougat',   axis:'y', dist:120, speed:2.8 },
      { x:3070, y:360, w:80, h:16, type:'ground',   axis:'x', dist:110, speed:3.2 },
      { x:3520, y:480, w:80, h:16, type:'nougat',   axis:'y', dist:100, speed:3.0 },
      { x:3950, y:340, w:80, h:16, type:'ground',   axis:'x', dist:120, speed:3.4 },
      { x:4340, y:420, w:80, h:16, type:'nougat',   axis:'y', dist:130, speed:3.2 },
      { x:4740, y:500, w:80, h:16, type:'ground',   axis:'x', dist:140, speed:3.6 },
      { x:5000, y:420, w:80, h:16, type:'nougat',   axis:'y', dist:80,  speed:3.4 },
    ],
    checkpoints: [
      { x:950,  y:260 },
      { x:2200, y:360 },
    ...
    collectibles: [
      ...[100,180,260,340,420,500,580,660,740,820,900,980,1060,1140,1220,
          ...
          2340,2420,2500,2580,2660,2740,2820,2900,2980,3060,3140,3220,3300,
          3380,3460,3540,3620,3700,3780,3860,3940,4020,4100,4180,4260,4340,
          4420,4500,4580,4660,4740,4820,4900,4980,5060,5140,5220,5300].map(
        (x,i) => ({ x, y: 200 + (i%5)*36 })
      ),
    ],
    powerups: [
      { x:500,  y:340, w:32, h:32, type:'doubleJump' },
      { x:960,  y:260, w:32, h:32, type:'speed'      },
      { x:1660, y:340, w:32, h:32, type:'dash'       },
      { x:2210, y:300, w:32, h:32, type:'shield'     },
      { x:2610, y:180, w:32, h:32, type:'ultra'      },
      { x:3260, y:340, w:32, h:32, type:'doubleJump' },
      { x:3860, y:300, w:32, h:32, type:'speed'      },
      { x:4160, y:220, w:32, h:32, type:'shield'     },
      { x:5110, y:340, w:32, h:32, type:'ultra'      },
    ],
    enemies: [
      { x:360,  y:460, type:'chocolate', range:120 },
      { x:560,  y:400, type:'hazelnut'             },
      { x:760,  y:340, type:'guard',     range:140 },
      { x:960,  y:280, type:'chocolate', range:140 },
      { x:1160, y:340, type:'hazelnut'             },
      { x:1360, y:400, type:'guard',     range:160 },
      { x:1560, y:460, type:'chocolate', range:160 },
      { x:1760, y:520, type:'hazelnut'             },
      { x:2010, y:460, type:'guard',     range:180 },
      { x:2210, y:380, type:'chocolate', range:180 },
      { x:2410, y:300, type:'hazelnut'             },
      { x:2610, y:240, type:'guard',     range:140 },
      { x:2810, y:300, type:'chocolate', range:200 },
      { x:3010, y:380, type:'hazelnut'             },
      { x:3210, y:460, type:'guard',     range:200 },
      { x:3660, y:440, type:'chocolate', range:140 },
      { x:3860, y:360, type:'hazelnut'             },
      { x:4060, y:280, type:'guard',     range:220 },
      { x:4260, y:360, type:'chocolate', range:220 },
      { x:4460, y:440, type:'hazelnut'             },
      { x:4660, y:520, type:'guard',     range:240 },
      { x:4910, y:440, type:'chocolate', range:240 },
      { x:5110, y:360, type:'guard',     range:260 },
    ],
    endZone: { x:5310, y:350, w:56, h:80 },
  },

  // ── NIVEAU 6 : Cosmos Sucré (boss final) ─────────────────────────
  {
    name: 'Cosmos Sucré',
    width: 6000, height: 680,
    bgTop: '#000008', bgBot: '#080020',
    spawn: { x: 80, y: 420 },
    platforms: [
      { x:0,    y:520, w:250, h:20, type:'candy'     },
      { x:300,  y:520, w:180, h:20, type:'chocolate' },
      { x:530,  y:520, w:180, h:20, type:'candy'     },
      { x:760,  y:520, w:180, h:20, type:'chocolate' },
      { x:990,  y:520, w:180, h:20, type:'candy'     },
      { x:1220, y:520, w:180, h:20, type:'chocolate' },
      { x:1450, y:520, w:180, h:20, type:'candy'     },
      { x:1680, y:520, w:180, h:20, type:'chocolate' },
      { x:1910, y:520, w:180, h:20, type:'candy'     },
      { x:2140, y:520, w:180, h:20, type:'chocolate' },
      { x:2370, y:520, w:180, h:20, type:'candy'     },
      { x:2600, y:520, w:180, h:20, type:'chocolate' },
      { x:2830, y:520, w:180, h:20, type:'candy'     },
      { x:3060, y:520, w:180, h:20, type:'chocolate' },
      { x:3290, y:520, w:180, h:20, type:'candy'     },
      { x:3520, y:520, w:180, h:20, type:'chocolate' },
      { x:3750, y:520, w:180, h:20, type:'candy'     },
      { x:3980, y:520, w:180, h:20, type:'chocolate' },
      { x:4210, y:520, w:180, h:20, type:'candy'     },
      { x:4440, y:520, w:180, h:20, type:'chocolate' },
      { x:4670, y:520, w:180, h:20, type:'candy'     },
      { x:4900, y:520, w:180, h:20, type:'chocolate' },
      { x:5130, y:520, w:180, h:20, type:'candy'     },
      { x:5360, y:520, w:180, h:20, type:'chocolate' },
      { x:5590, y:520, w:410, h:20, type:'candy'     },
      // Haute altitude
      { x:200,  y:400, w:100, h:16, type:'candy'     },
      { x:450,  y:330, w:100, h:16, type:'chocolate' },
      { x:700,  y:260, w:100, h:16, type:'candy'     },
      { x:950,  y:330, w:100, h:16, type:'chocolate' },
      { x:1200, y:400, w:100, h:16, type:'candy'     },
      { x:1500, y:330, w:100, h:16, type:'chocolate' },
      { x:1800, y:260, w:100, h:16, type:'candy'     },
      { x:2100, y:330, w:100, h:16, type:'chocolate' },
      { x:2400, y:400, w:100, h:16, type:'candy'     },
      { x:2700, y:330, w:100, h:16, type:'chocolate' },
      { x:3000, y:260, w:100, h:16, type:'candy'     },
      { x:3300, y:330, w:100, h:16, type:'chocolate' },
      { x:3600, y:400, w:100, h:16, type:'candy'     },
      { x:3900, y:330, w:100, h:16, type:'chocolate' },
      { x:4200, y:260, w:100, h:16, type:'candy'     },
      { x:4500, y:330, w:100, h:16, type:'chocolate' },
      { x:4800, y:400, w:100, h:16, type:'candy'     },
      { x:5100, y:330, w:100, h:16, type:'chocolate' },
      { x:5400, y:260, w:100, h:16, type:'candy'     },
    ],
    moving: [
      // 16 plateformes mobiles — toutes rapides
      { x:250,  y:490, w:80, h:16, type:'candy',    axis:'x', dist:40,  speed:2.5 },
      { x:480,  y:490, w:80, h:16, type:'chocolate',axis:'y', dist:120, speed:3.0 },
      { x:710,  y:480, w:80, h:16, type:'candy',    axis:'x', dist:50,  speed:3.0 },
      { x:940,  y:480, w:80, h:16, type:'chocolate',axis:'y', dist:140, speed:3.2 },
      { x:1170, y:470, w:80, h:16, type:'candy',    axis:'x', dist:60,  speed:3.2 },
      { x:1400, y:470, w:80, h:16, type:'chocolate',axis:'y', dist:150, speed:3.4 },
      { x:1630, y:460, w:80, h:16, type:'candy',    axis:'x', dist:70,  speed:3.4 },
      { x:1860, y:460, w:80, h:16, type:'chocolate',axis:'y', dist:160, speed:3.6 },
      { x:2090, y:450, w:80, h:16, type:'candy',    axis:'x', dist:80,  speed:3.6 },
      { x:2320, y:450, w:80, h:16, type:'chocolate',axis:'y', dist:170, speed:3.8 },
      { x:2550, y:440, w:80, h:16, type:'candy',    axis:'x', dist:90,  speed:3.8 },
      { x:2780, y:440, w:80, h:16, type:'chocolate',axis:'y', dist:180, speed:4.0 },
      { x:3010, y:430, w:80, h:16, type:'candy',    axis:'x', dist:100, speed:4.0 },
      { x:3470, y:430, w:80, h:16, type:'chocolate',axis:'y', dist:180, speed:4.0 },
      { x:3930, y:420, w:80, h:16, type:'candy',    axis:'x', dist:110, speed:4.2 },
      { x:4390, y:420, w:80, h:16, type:'chocolate',axis:'y', dist:190, speed:4.2 },
      { x:4850, y:410, w:80, h:16, type:'candy',    axis:'x', dist:120, speed:4.4 },
      { x:5310, y:410, w:80, h:16, type:'chocolate',axis:'y', dist:200, speed:4.4 },
    ],
    checkpoints: [
      { x:700,  y:480 },
      { x:1800, y:480 },
    ...
    collectibles: [
      ...[100,180,260,340,420,500,580,660,740,820,900,980,1060,1140,1220,
          ...
          2340,2420,2500,2580,2660,2740,2820,2900,2980,3060,3140,3220,3300,
          3380,3460,3540,3620,3700,3780,3860,3940,4020,4100,4180,4260,4340,
          4420,4500,4580,4660,4740,4820,4900,4980,5060,5140,5220,5300,5380,
          5460,5540,5620,5700,5780,5860,5940].map(
        (x,i) => ({ x, y: 210 + (i%6)*38 })
      ),
    ],
    powerups: [
      { x:460,  y:290, w:32, h:32, type:'doubleJump' },
      { x:960,  y:290, w:32, h:32, type:'speed'      },
      { x:1460, w:32, h:32, y:290, type:'dash'       },
      { x:1960, y:220, w:32, h:32, type:'shield'     },
      { x:2460, y:290, w:32, h:32, type:'ultra'      },
      { x:2960, y:220, w:32, h:32, type:'doubleJump' },
      { x:3460, y:290, w:32, h:32, type:'speed'      },
      { x:3960, y:290, w:32, h:32, type:'dash'       },
      { x:4460, y:220, w:32, h:32, type:'shield'     },
      { x:4960, y:290, w:32, h:32, type:'ultra'      },
      { x:5460, y:220, w:32, h:32, type:'doubleJump' },
    ],
    enemies: [
      { x:320,  y:500, type:'guard',     range:180 },
      { x:550,  y:500, type:'chocolate', range:160 },
      { x:780,  y:500, type:'hazelnut'             },
      { x:1010, y:500, type:'guard',     range:200 },
      { x:1240, y:500, type:'chocolate', range:180 },
      { x:1470, y:500, type:'hazelnut'             },
      { x:1700, y:500, type:'guard',     range:220 },
      { x:1930, y:500, type:'chocolate', range:200 },
      { x:2160, y:500, type:'hazelnut'             },
      { x:2390, y:500, type:'guard',     range:240 },
      { x:2620, y:500, type:'chocolate', range:220 },
      { x:2850, y:500, type:'hazelnut'             },
      { x:3080, y:500, type:'guard',     range:260 },
      { x:3310, y:500, type:'chocolate', range:240 },
      { x:3540, y:500, type:'hazelnut'             },
      { x:3770, y:500, type:'guard',     range:280 },
      { x:4000, y:500, type:'chocolate', range:260 },
      { x:4230, y:500, type:'hazelnut'             },
      { x:4460, y:500, type:'guard',     range:300 },
      { x:4690, y:500, type:'chocolate', range:280 },
      { x:4920, y:500, type:'hazelnut'             },
      { x:5150, y:500, type:'guard',     range:300 },
      { x:5380, y:500, type:'chocolate', range:300 },
      { x:5610, y:500, type:'guard',     range:300 },
    ],
    endZone: { x:5910, y:390, w:56, h:80 },
  },
];
...
// QUIZZES
// ============================================================
// Pool de 25 questions — une est tirée aléatoirement à chaque fin de niveau
const QUIZ_POOL = [
  {
    q: 'Quelle marque fabrique les Schoko-Bons ?',
    a: ['Nestlé', 'Haribo', 'Kinder (Ferrero)', 'Mars'],
    ok: 2,
    expl: 'Kinder est une marque de Ferrero, fondée en 1946 en Italie. "Kinder" signifie "enfants" en allemand !',
  },
  {
    q: 'De quoi est composée la farce d\'un Schoko-Bon ?',
    a: ['Caramel et praliné', 'Noisette et chocolat blanc', 'Guimauve vanillée', 'Confiture de fraise'],
    ok: 1,
    expl: 'La farce des Schoko-Bons est un mélange onctueux de noisette broyée et de chocolat blanc — c\'est ça le secret !',
  },
  {
    q: 'En quelle année la société Ferrero a-t-elle été fondée ?',
    a: ['1934', '1946', '1958', '1962'],
    ok: 1,
    expl: 'Pietro Ferrero a fondé Ferrero en 1946 à Alba, en Italie, après la Seconde Guerre Mondiale.',
  },
  {
    q: 'Que signifie le mot "Kinder" en allemand ?',
    a: ['Doux', 'Chocolat', 'Enfants', 'Bonheur'],
    ok: 2,
    expl: '"Kinder" signifie "enfants" en allemand. La marque vise les produits sucrés destinés aux plus jeunes !',
  },
  {
    q: 'Quelle est la forme caractéristique d\'un Schoko-Bon ?',
    a: ['Carrée', 'Ronde parfaite', 'Ovale (comme un œuf)', 'En forme de cœur'],
    ok: 2,
    expl: 'Les Schoko-Bons ont une forme ovale distinctive, avec leur emballage en papier torsadé aux deux extrémités.',
  },
  {
    q: 'Dans quel pays est née la marque Ferrero ?',
    a: ['France', 'Belgique', 'Suisse', 'Italie'],
    ok: 3,
    expl: 'Ferrero est né à Alba, dans la région du Piémont en Italie, connue pour ses noisettes de qualité exceptionnelle.',
  },
  {
    q: 'Quel autre produit très célèbre appartient à Ferrero ?',
    a: ['Milka', 'Nutella', 'Toblerone', 'Bounty'],
    ok: 1,
    expl: 'Le Nutella est lui aussi une création de Ferrero ! Pietro Ferrero l\'inventa en 1946 sous le nom de "Pasta Gianduja".',
  },
  {
    q: 'De quel type de chocolat est recouverte la coque des Schoko-Bons ?',
    a: ['Chocolat noir 70%', 'Chocolat blanc', 'Chocolat au lait', 'Chocolat ruby'],
    ok: 2,
    expl: 'La coque des Schoko-Bons est en chocolat au lait, fine et légèrement croquante, qui fond en bouche.',
  },
  {
    q: 'Quel autre produit Kinder est célèbre pour sa surprise jouet ?',
    a: ['Kinder Délice', 'Kinder Bueno', 'Kinder Surprise', 'Kinder Maxi'],
    ok: 2,
    expl: 'Le Kinder Surprise, lancé en 1974, contient un jouet en plastique caché à l\'intérieur — il est interdit aux USA !',
  },
  {
    q: 'Quelle région italienne fournit les noisettes utilisées par Ferrero ?',
    a: ['Toscane', 'Sicile', 'Piémont', 'Lombardie'],
    ok: 2,
    expl: 'Le Piémont est réputé pour ses noisettes "Tonda Gentile" d\'une qualité exceptionnelle, base des produits Ferrero.',
  },
  {
    q: 'Que signifie "Schoko" en allemand ?',
    a: ['Sucre', 'Chocolat', 'Bonbon', 'Douceur'],
    ok: 1,
    expl: '"Schoko" est l\'abréviation de "Schokolade" qui signifie "chocolat" en allemand. Schoko-Bon = Bon chocolat !',
  },
  {
    q: 'Les Schoko-Bons sont-ils disponibles en version chocolat blanc ?',
    a: ['Oui, depuis 2010', 'Oui, en édition limitée', 'Non, seulement chocolat au lait', 'Oui, toute l\'année'],
    ok: 2,
    expl: 'Les Schoko-Bons classiques sont uniquement en chocolat au lait. La farce intérieure contient du chocolat blanc.',
  },
  {
    q: 'Quel est le produit emblématique de Ferrero pour les fêtes ?',
    a: ['Ferrero Collection', 'Raffaello', 'Rocher Ferrero', 'Kinder Maxi'],
    ok: 2,
    expl: 'Le Rocher Ferrero — boule chocolatée au praliné enrobée de gaufrette et noisettes — est le symbole des fêtes !',
  },
  {
    q: 'Combien de temps faut-il pour manger un Schoko-Bon si on le laisse fondre ?',
    a: ['5 secondes', '15 à 20 secondes', '1 minute', '2 minutes'],
    ok: 1,
    expl: 'La fine coque au chocolat au lait fond en environ 15-20 secondes dans la bouche — un vrai plaisir !',
  },
  {
    q: 'Quelle est la couleur principale du packaging des Schoko-Bons ?',
    a: ['Bleu et or', 'Vert et blanc', 'Rouge, blanc et bleu', 'Orange et marron'],
    ok: 2,
    expl: 'Le packaging des Schoko-Bons arbore les couleurs rouge et blanc caractéristiques de la marque Kinder, avec du bleu.',
  },
  {
    q: 'Quel concurrent direct des Schoko-Bons existe chez Nestlé ?',
    a: ['Smarties', 'Chokinis', 'KitKat Bouchées', 'Quality Street'],
    ok: 1,
    expl: 'Les Chokinis de Nestlé sont souvent cités comme concurrents des Schoko-Bons, avec une farce crémeuse similaire.',
  },
  {
    q: 'En quelle saison les ventes de Schoko-Bons sont-elles les plus élevées ?',
    a: ['L\'été', 'Le printemps uniquement', 'Noël et Pâques', 'La rentrée scolaire'],
    ok: 2,
    expl: 'Comme beaucoup de confiseries au chocolat, les Schoko-Bons se vendent énormément à Noël et à Pâques !',
  },
  {
    q: 'Quel est l\'autre nom du "Kinder Bueno" en Allemagne lors de son lancement ?',
    a: ['Kinder Schoko', 'Kinder Classico', 'Kinder Merendero', 'Kinder Bravo'],
    ok: 2,
    expl: 'En Italie, le Kinder Bueno était initialement vendu sous le nom "Kinder Merendero" avant d\'adopter le nom "Bueno".',
  },
  {
    q: 'Quelle est approximativement la teneur en cacao de la coque des Schoko-Bons ?',
    a: ['12%', '31%', '55%', '70%'],
    ok: 1,
    expl: 'La coque chocolat au lait des Schoko-Bons contient environ 31% de cacao — typique du chocolat au lait de qualité.',
  },
  {
    q: 'Qui est le fondateur de la société Ferrero ?',
    a: ['Giovanni Ferrero', 'Luigi Ferrero', 'Pietro Ferrero', 'Marco Ferrero'],
    ok: 2,
    expl: 'Pietro Ferrero, pâtissier piémontais, fonda l\'entreprise en 1946. Son fils Michele développa ensuite la marque Kinder.',
  },
  {
    q: 'Dans quel pays les Schoko-Bons sont-ils principalement fabriqués ?',
    a: ['France', 'Allemagne', 'Belgique', 'Italie'],
    ok: 3,
    expl: 'Les Schoko-Bons sont fabriqués en Italie, dans les usines Ferrero du Piémont, qui emploient des milliers de personnes.',
  },
  {
    q: 'Quel arbre fournit la matière première principale de la farce Schoko-Bons ?',
    a: ['L\'amandier', 'Le noisetier', 'Le cacaoyer', 'Le cocotier'],
    ok: 1,
    expl: 'Le noisetier fournit les noisettes utilisées dans la farce. Ferrero est le premier acheteur mondial de noisettes !',
  },
  {
    q: 'Quelle proportion des noisettes mondiales Ferrero achète-t-elle chaque année ?',
    a: ['5%', '13%', '25%', '40%'],
    ok: 2,
    expl: 'Ferrero achète environ 25% de la production mondiale de noisettes chaque année — un chiffre colossal !',
  },
  {
    q: 'Les Schoko-Bons contiennent-ils du lait en poudre ?',
    a: ['Non, zéro produit laitier', 'Oui, dans la farce et la coque', 'Seulement dans la farce', 'Seulement dans la coque'],
    ok: 1,
    expl: 'Oui ! Le lait en poudre est présent à la fois dans la coque chocolat au lait et dans la farce crémeuse aux noisettes.',
  },
  {
    q: 'Quelle lettre iconique apparaît sur le ruban du packaging des Schoko-Bons ?',
    a: ['F comme Ferrero', 'K comme Kinder', 'S comme Schoko', 'B comme Bon'],
    ok: 1,
    expl: 'Le "K" de Kinder est présent sur l\'emballage. La bandelette bleue et blanche avec "kinder" est la signature de la marque.',
  },
];
// Compatibilité : alias legacy (handleQuiz/drawQuiz utilise currentQuiz)
const QUIZZES = QUIZ_POOL;
...
function loadLevel(idx) {
  ...
  lPowerUps      = (levelData.powerups || levelData.powerUps || []).map(p => new PowerUpItem(p.x, p.y, p.type));
  lEndZone       = levelData.endZone || levelData.end;
  lMoving      = (levelData.moving  || []).map(m => new MovingPlatform(m.x,m.y,m.w,m.h,m.type,m.axis,m.dist,m.speed));
  lCheckpoints = (levelData.checkpoints || []).map(c => new Checkpoint(c.x, c.y));
  checkpointHit  = false;
  checkpointX    = levelData.spawn.x;
  checkpointY    = levelData.spawn.y;
  flashTimer     = 0;
  levelTransition = 30;
...
function drawQuiz() {
  ...
  const q = currentQuiz || QUIZ_POOL[0];
  ctx.textAlign='center';
...
function drawEndScreen() {
  ...
  const ctaHover = _endMouseX >= END_CTA.x && _endMouseX < END_CTA.x+END_CTA.w &&
                   _endMouseY >= END_CTA.y && _endMouseY < END_CTA.y+END_CTA.h;
  const ctaScale = 1 + 0.018 * Math.sin(frameCount * 0.07);
  ctx.save();
  ctx.translate(W/2, END_CTA.y + END_CTA.h/2);
  if (ctaHover) ctx.scale(1.04, 1.04); else ctx.scale(ctaScale, ctaScale);
  ctx.translate(-W/2, -(END_CTA.y + END_CTA.h/2));
  ctx.shadowColor = '#E2001A'; ctx.shadowBlur = ctaHover ? 28 : 12;
  ctx.save();
  _clipRR(END_CTA.x, END_CTA.y, END_CTA.w, END_CTA.h, 14); ctx.clip();
  const ctaG = ctx.createLinearGradient(END_CTA.x, END_CTA.y, END_CTA.x, END_CTA.y+END_CTA.h);
  ctaG.addColorStop(0, ctaHover ? '#FF2233' : '#E2001A');
  ctaG.addColorStop(1, ctaHover ? '#AA0010' : '#8B0010');
  ctx.fillStyle = ctaG; ctx.fillRect(END_CTA.x, END_CTA.y, END_CTA.w, END_CTA.h);
  ctx.fillStyle = 'rgba(255,255,255,0.18)'; ctx.fillRect(END_CTA.x, END_CTA.y, END_CTA.w, END_CTA.h*0.42);
  ctx.restore();
  ctx.strokeStyle = 'rgba(255,180,180,0.7)'; ctx.lineWidth = 2;
  _clipRR(END_CTA.x, END_CTA.y, END_CTA.w, END_CTA.h, 14); ctx.stroke();
  ctx.shadowBlur = 0;
  ctx.fillStyle = '#FFFFFF'; ctx.font = 'bold 20px Arial';
  ctx.fillText('🍫  Découvrir les vrais Schoko-Bons  →', W/2, END_CTA.y + END_CTA.h/2 + 7);
  ctx.restore();

  // ── Bouton secondaire Rejouer (utilise END_RPL défini globalement) ──
  const rpHover = _endMouseX >= END_RPL.x && _endMouseX < END_RPL.x+END_RPL.w &&
                  _endMouseY >= END_RPL.y && _endMouseY < END_RPL.y+END_RPL.h;
  ctx.save();
  ctx.shadowColor = 'rgba(92,51,23,0.5)'; ctx.shadowBlur = rpHover ? 18 : 6;
  ctx.save();
  _clipRR(END_RPL.x, END_RPL.y, END_RPL.w, END_RPL.h, 12); ctx.clip();
  const rpG = ctx.createLinearGradient(END_RPL.x, END_RPL.y, END_RPL.x, END_RPL.y+END_RPL.h);
  rpG.addColorStop(0, rpHover ? '#7A4520' : '#5C3317');
  rpG.addColorStop(1, rpHover ? '#3E1A08' : '#2A1005');
  ctx.fillStyle = rpG; ctx.fillRect(END_RPL.x, END_RPL.y, END_RPL.w, END_RPL.h);
  ctx.fillStyle = 'rgba(255,255,255,0.12)'; ctx.fillRect(END_RPL.x, END_RPL.y, END_RPL.w, END_RPL.h*0.4);
  ctx.restore();
  ctx.strokeStyle = rpHover ? 'rgba(255,180,100,0.9)' : 'rgba(200,140,60,0.5)';
  ctx.lineWidth = 1.5;
  _clipRR(END_RPL.x, END_RPL.y, END_RPL.w, END_RPL.h, 12); ctx.stroke();
  ctx.shadowBlur = 0;
...
function drawGameOver() {
  ...
  const vr = ctx.createRadialGradient(W/2,H/2,H*0.1,W/2,H/2,H*0.9);
  vr.addColorStop(0,'rgba(0,0,0,0)');
  vr.addColorStop(1,`rgba(180,0,0,${0.4+0.15*Math.sin(frameCount*0.06)})`);
  ctx.fillStyle=vr; ctx.fillRect(0,0,W,H);

  // Shattered chocolate pieces falling
  ctx.save();
  for (let i=0;i<12;i++) {
    const px=W*0.15+i*(W*0.06), py=(frameCount*0.8+i*55)%(H+80)-40;
    const rot=(frameCount*0.02+i)*1.3;
    ctx.save(); ctx.translate(px,py); ctx.rotate(rot);
    ctx.fillStyle=`rgba(80,30,5,${0.5+0.3*Math.sin(i)})`;
    ctx.fillRect(-12,-8,24,16); ctx.restore();
  }
  ctx.restore();

  // Title
  ctx.textAlign='center';
  ctx.shadowColor='#E8001C'; ctx.shadowBlur=30;
  ctx.fillStyle='#E8001C'; ctx.font='bold 64px Arial';
  ctx.fillText('GAME OVER', W/2, H/2-70);
  ctx.shadowBlur=0;

  // Score panel
  _clipRR(W/2-160, H/2-28, 320, 56, 12);
  ctx.save(); ctx.clip();
  const spg=ctx.createLinearGradient(W/2-160,H/2-28,W/2+160,H/2+28);
  spg.addColorStop(0,'rgba(80,10,10,0.85)'); spg.addColorStop(1,'rgba(40,5,5,0.85)');
  ctx.fillStyle=spg; ctx.fillRect(W/2-160,H/2-28,320,56);
  ctx.restore();
  ctx.strokeStyle='rgba(230,0,0,0.6)'; ctx.lineWidth=2;
  _clipRR(W/2-160, H/2-28, 320, 56, 12); ctx.stroke();
  ctx.fillStyle='#FFD700'; ctx.font='bold 26px Arial';
  ctx.fillText(`Score : ${score}`, W/2, H/2+8);

  // Lives lost info
  ctx.fillStyle='rgba(255,180,180,0.9)'; ctx.font='16px Arial';
  ctx.fillText('Plus de vies… retente ta chance !', W/2, H/2+52);

  // Blinking restart prompt
  if (Math.floor(frameCount/28)%2===0) {
    ctx.fillStyle='rgba(255,255,255,0.85)'; ctx.font='bold 18px Arial';
    ctx.fillText('▶  ENTRÉE / ESPACE / TOUCHER pour recommencer', W/2, H/2+92);
  }
  ctx.textAlign='left';
  if (just('Enter')||just('Space')) { sfx.menuClick(); gameState='menu'; }
}
...
function drawWin() {
  ...
  g.addColorStop(0,'#1A0050'); g.addColorStop(0.5,'#3A0080'); g.addColorStop(1,'#180030');
  ctx.fillStyle=g; ctx.fillRect(0,0,W,H);

  // Stars / sparkles background
  for (let i=0;i<40;i++) {
    const sx=(i*137+frameCount*0.4)%W;
    const sy=(i*89)%(H*0.7);
    const sr=1+Math.sin(frameCount*0.07+i)*0.8;
    ctx.fillStyle=`rgba(255,220,100,${0.3+0.4*Math.sin(frameCount*0.05+i*0.4)})`;
    ctx.beginPath(); ctx.arc(sx,sy,sr,0,Math.PI*2); ctx.fill();
  }

  // Confetti
  for (let i=0;i<30;i++) {
    const cx2=((i*97+frameCount*1.8)%(W+60))-30;
    const cy2=((i*61+frameCount*0.9)%(H+60))-30;
    const cr=(frameCount*0.04+i)*1.5;
    const cc=['#FFD700','#FF4466','#44FFAA','#4488FF','#FF8844'][i%5];
    ctx.save(); ctx.translate(cx2,cy2); ctx.rotate(cr);
    ctx.fillStyle=cc; ctx.fillRect(-6,-4,12,8); ctx.restore();
  }
  ...
  ctx.fillText('VICTOIRE !', W/2, 115);
  ctx.shadowBlur=0;

  ctx.fillStyle='rgba(255,230,180,0.95)'; ctx.font='22px Arial';
  ctx.fillText('Tu as terminé tous les niveaux !', W/2, 162);

  // Score panel
  ctx.save();
  _clipRR(W/2-200, 188, 400, 80, 16); ctx.clip();
  const sp=ctx.createLinearGradient(W/2-200,188,W/2+200,268);
  sp.addColorStop(0,'rgba(100,50,0,0.85)'); sp.addColorStop(1,'rgba(50,20,0,0.85)');
  ctx.fillStyle=sp; ctx.fillRect(W/2-200,188,400,80);
  ctx.restore();
  ctx.strokeStyle='rgba(255,200,50,0.7)'; ctx.lineWidth=2;
  _clipRR(W/2-200, 188, 400, 80, 16); ctx.stroke();
  ctx.fillStyle='#FFFFFF'; ctx.font='bold 20px Arial';
  ctx.fillText(`Score final : ${score}`, W/2, 222);
  if (score > highScore) { highScore = score; }
  ctx.fillStyle='#FFD700'; ctx.font='16px Arial';
  ctx.fillText(`Record : ${highScore}`, W/2, 254);

  // Star rating
  const stars = score >= 3000 ? 3 : score >= 1500 ? 2 : 1;
  const labels = ['Apprenti Gourmand', 'Expert Sucré !', 'Maître Chocolatier !'];
  for (let s=0;s<3;s++) {
    const sx=W/2 - 60 + s*60;
    const sy=295;
    const lit = s < stars;
    ctx.fillStyle = lit ? '#FFD700' : 'rgba(255,255,255,0.2)';
    ctx.font = '36px Arial';
    ctx.fillText('★', sx, sy);
  }
  ctx.fillStyle = stars===3 ? '#FFD700' : 'rgba(255,255,255,0.7)';
  ctx.font = 'bold 20px Arial';
  ctx.fillText(labels[stars-1], W/2, 330);

  // Blinking prompt
  if (Math.floor(frameCount/30)%2===0) {
    ctx.fillStyle='rgba(255,255,255,0.88)'; ctx.font='bold 18px Arial';
    ctx.fillText('▶  ENTRÉE / ESPACE / TOUCHER pour rejouer', W/2, 390);
  }
  ctx.textAlign='left';
  if (just('Enter')||just('Space')) { sfx.menuClick(); score=0; lives=3; loadLevel(0); }
}
...
function loop() {
  ...
  lMoving.forEach(m => m.update());
  lCheckpoints.forEach(c => c.update());
  ...
  if (player && !player.dead) {
    for (const cp of lCheckpoints) {
      if (!cp.activated && overlap(
        { x: player.x, y: player.y, w: player.w, h: player.h },
        { x: cp.x, y: cp.y, w: cp.w, h: cp.h }
      )) {
        cp.activated = true; checkpointHit = true;
        checkpointX = cp.x; checkpointY = cp.y - player.h;
        burst(cp.x + 14, cp.y, '#FFD700', 14, 4);
        notify('Checkpoint !', '#FFD700');
        sfx.collect();
      }
    }
  }

  // Player rides moving platforms
  if (player && player.onGround) {
    for (const m of lMoving) {
      if (overlap({ x: player.x, y: player.y + player.h - 4, w: player.w, h: 8 },
  ...
  if (flashTimer > 0) {
    ctx.fillStyle = `rgba(220,0,0,${flashTimer / 12 * 0.35})`;
    ctx.fillRect(0, 0, W, H);
    flashTimer--;
  }
  // Level transition fade-in
  if (levelTransition > 0) {
    ctx.fillStyle = `rgba(0,0,0,${levelTransition / 30})`;
    ctx.fillRect(0, 0, W, H);
    levelTransition--;
  }
